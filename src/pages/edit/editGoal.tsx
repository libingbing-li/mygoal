		...
		return (
          ...
          />
					<textarea 
						...
						onChange={(e) => this.changeModelState('description', e.target.value)}
            placeholder='请输入目标描述'
					></textarea>
				</div>
        <div style={{display: history.location.query?.timeId === null ? 'none' : 'block'}}>
          {this.props.data?.finishDescription}
        </div>
			</div>
		);
	}
