...
class dataBox extends React.Component<IProps  & {infoNote: string; nowNote: string; dispatch: any}> {
  ...
  renderBox = () => {
    ...
    // }
    div = (
      <div className={style.dataBox} id="dataBox" style={{bottom: '-50vh'}}>
        <div className={style.btns}>
        <CloseOutlined onClick={this.props.close}/>
        <EditOutlined onClick={() => history.push(`./editGoal?timeId=${this.props.data.timeId}`)}/>
        </div>
        ...
        <div className={style.description}>{this.props.data.description}</div>
        <div className={style.finish}>{this.props.data.finishDescription}111111111</div>
      </div>
    );
    return div;
  }
